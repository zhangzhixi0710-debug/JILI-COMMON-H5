<template>
    <div class="single-images-list">
        ...
        <template v-if="keyValue === 1">
            <div class="free-bonus-game">
                <div class="free-bonus-game-span">
                    <span>{{ $t("Number") }} 1：</span>
                    <span>{{ item.firstPrice }}</span>
                </div>
                <div class="free-bonus-game-span">
                    <span>{{ $t("TotalGoldPrice") }}：</span>
                    <span>{{ item.totalGoldPrice }}</span>
                </div>
                <div class="free-bonus-game-span">
                    <span>{{ $t("Get") + item.getFreeSpins + $t("FreeGames") }}</span>
                </div>
            </div>
        </template>
        ...
        <template v-else-if="item.PlateSymbolExtend">
            <div class="single-images-contain">
                ...
                <div class="single-images-footer" v-if="item.AwardDataVec?.length">
                    ...
                    >
                        <img v-if="opt.img" :src="opt.img" alt="" />
                        <span v-if="opt.Index">{{ opt.Index }}</span>
                    </div>
                </div>
            </div>
        </template>
        ...
        <template v-if="isFreeGame">
            <template v-if="item.isFrist">
                <div class="single-images-event" v-if="item.RoundWin">
            ...
            <div
                ...
                v-else-if="item.RoundWin && item.BarWin != item.RoundWin"
            >
            ...
            <div v-if="item.BarWin" class="single-images-event">
                <span>{{ $t("GoldBarWin") }}</span>
                <span>{{ formatNumber(item.BarWin) }}</span>
            </div>
        </template>
...
export default {
    ...
    computed: {
        ...
        gameTitle() {
            ...
            if (this.keyValue === 1) {
                return this.$t("i18_BonusGame");
            }

            return this.item.isFrist ? this.$t("MainGame") : this.$t("FreeGame");
        },
    },
    mounted() {},
    methods: {
