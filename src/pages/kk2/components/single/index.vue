<template>
    <div class="single-images-list">
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <div v-if="item.RoundWin && index === config.plates.length - 1" class="single-images-event">
                <span>{{ $t("MainGameWin") }}</span>
                <span>{{ formatNumber(item.RoundWin) || 0 }}</span>
            </div>

            <div v-if="isFreeGame && index === config.plates.length - 1" class="single-images-event">
                <span>{{ $t("Get") + 1 + $t("FreeGames") }}</span>
            </div>
        </div>
    </div>
</template>
