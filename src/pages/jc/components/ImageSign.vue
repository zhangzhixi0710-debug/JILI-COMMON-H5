<template>
    <div class="single-images-list">
        ...
        <template v-if="item.PlateSymbolExtend">
            <div class="single-images-contain">
                ...
                <div
                    ...
                    :class="{ 'single-images-middle-free': !item.isFrist }"
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                ...
                                v-if="item.MultImages.length"
                                :class="{
                                    'single-images-num-active': isAwardActive(i, imgIndex),
                                }"
                            >
                                <img
                                    v-if="im"
                                    v-for="(im, numIndex) in item.MultImages[i][imgIndex]"
                                    :key="numIndex"
                                    :class="{
                                        'single-images-num-single': item.MultImages[i][imgIndex],
                                    }"
                                    :style="{
                                        'max-width':
                                            (100 / item.MultImages[i][imgIndex].length).toFixed(0) +
                                            '%',
                                    }"
                                    :src="im"
                                />
        ...
        <template v-if="isFreeGame" class="single-images-event">
            <div v-if="item.RoundWin" class="single-images-event">
                <span v-if="item.isFrist">{{ $t("MainGameWin") }}</span>
                <span v-else>{{ $t("Respin_Win_min") }}</span>
                <span>{{ formatNumber(item.RoundWin) }}</span>
            </div>
        </template>

        <div v-else="item.RoundWin" class="single-images-event">
