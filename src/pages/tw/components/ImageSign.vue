<template>
    <div class="single-images-list">
        <div v-for="(item, index) in config.rounds" :key="index">
            ...
            <template v-if="isFreeGame">
                <div class="single-images-event" v-if="config?.rounds?.length - 1 === index">
                    <span>{{ $t("MainGameWin") }}</span>
                    <span>0</span>
                </div>

                <div class="single-images-event">
                    <span v-if="item.RoundWin">{{ $t("Respin_Win_min") }}</span>
