                    ...
                    >
                        <div class="div-table">
                            <div class="div-table-row div-table-header">
                                <div class="div-table-cell">{{ $t("i18_SpecialAward") }}</div>
                                <div class="div-table-cell">{{ $t("i18_RoundNumber") }}</div>
                                <div class="div-table-cell">{{ $t("i18_NetWorth") }}</div>
                                <div class="div-table-cell">{{ $t("i18_NiuNiuAnte") }}</div>
                                <div class="div-table-cell">{{ $t("i18_Result_Win") }}</div>
                            ...
                            >
                                ...
                                <div class="div-table-cell">{{ item.Bet }}</div>
                                <div class="div-table-cell">{{ item.Win }}</div>
                                <div class="div-table-cell">
...
export default {
    ...
    methods: {
        ...
        async onLoad() {
            ...
            this.loading = true;
            try {
                const res = await Detail(this.gameId.toUpperCase(), { ...this.searchForm });
                const data = res?.Data || [];
                if (data.length) {
        ...
        async onRowClick(item) {
            ...
            try {
                const res = await HistoryItem(this.gameId.toUpperCase(), {
                ...
                if (decoded?.length) {
                    decoded.forEach(item => {
                        if (item.Odd?.includes(",")) {
                            const odds = item.Odd.split(",");
                            const fishes = item.Fish.split(",");
                            const wins = item.Win.split(",");
                ...
                console.log(JSON.stringify(decoded), JSON.stringify(result));
                if (!result) return;
