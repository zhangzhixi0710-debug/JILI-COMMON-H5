    ...
    <div class="single-container single">
        ...
        <div class="single-footer-data">
            ...
            <div class="single-table-sort">
                <el-checkbox class="custom-checkbox" v-model="sortCheck">{{
                    $t("WinSort")
                }}</el-checkbox>
            </div>
        </div>
                ...
                <div>
                    <div class="div-table">
                        <div class="div-table-row div-table-header">
                            ...
                            <div class="div-table-cell">{{ $t("i18_Win") }}</div>
                            <div class="div-table-cell">
                                {{ $t("i18_Remark") }}
                            </div>
                        </div>
                        <div class="div-table-row" v-for="(item, index) in RoundQueue" :key="index">
                            <div class="div-table-cell">
                                <img v-if="item.img" :src="item.img" />
                            </div>
                            <div class="div-table-cell">{{ item.Odd }}</div>
    ...
    methods: {
        ...
        async initData() {
            ...
            this.RoundQueue.forEach(fish => {
                let name = this.gameId;
                if (this.noJILINew.includes(this.gameId)) {
                    name = this.noJILINewName;
                }
                
                try {
                    fish.img = require(`@/assets/images/intro/${name}/${fish.Fish}.webp`);
                } catch (error) {}
                const bullet = this.filterBulletImageId(fish.Remark);

                if (bullet) {
                    fish.bulletImg = require(`@/assets/images/intro/${name}/${bullet}`);
                }
            });
        ...
        filterBulletImageId(name) {
            switch (name) {
                case "Normal":
                    return "";
                case "Harpoon":
                    return "bullet_2.webp";
                case "Cannon":
                    return "bullet_3.webp";
                case "Bomb":
                    return "bullet_6.webp";
                case "Energy":
                    return "bullet_8.webp";
                default:
                    return "";
            }
        },
