<template>
    <div class="single-images-list">
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <div v-if="config.FreeTotalRound" class="single-images-event">
                <span>{{ $t("Get") + config.FreeTotalRound + $t("FreeGames") }}</span>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        gameTitle() {
            return this.$t("FreeGame") + " " + `${this.keyValue + 1 + "/" + this.total}`;
        },
    },
    methods: {
        showHeader(uuid) {
