        ...
        <template v-if="item.PlateSymbolExtend">
            <div class="single-images-contain">
                        ...
                        >
                            ...
                            <div
                                :class="{
                                    ...
                                    'single-images-num': !item.BottomPlateIndex[i][imgIndex],
                                    'single-images-num-bottom': item.BottomPlateIndex[i][imgIndex],
                                }"
                ...
                <div class="single-images-footer" v-if="item.AwardDataVec?.length">
                    ...
                    >
                        <img :src="opt.Img" alt="" />
                        <span>{{ item.HasExpand ? "JP" : opt.Index }}</span>
                    </div>
                </div>
            </div>
        </template>

        <template v-if="item.HasExpand" class="single-images-event">
            <div v-if="item.RoundWin" class="single-images-event">
                <span v-if="item.isFrist">{{ $t("MainGameWin") }}</span>
                <span v-else>{{ $t("JPWin") }}：</span>
                <span>{{ formatNumber(item.RoundWin) }}</span>
            </div>

            <div v-if="item.isFrist" class="single-images-event">
                <span>{{ $t("GetExpansion") }}</span>
            </div>
        </template>

        <div v-else-if="item.RoundWin && BonusTotalWin" class="single-images-event">
            <span>{{ $t("BonusWin") }}</span>
            <span>{{ formatNumber(BonusTotalWin) }}</span>
        </div>
...
export default {
    ...
    props: {
        ...
        BonusTotalWin: { type: Number, default: 0 },
    },
    data() {
        return {
            changeList: {},
