    ...
    <div class="single-images-list">
        ...
        <template v-if="item.PlateSymbolExtend">
            <div class="single-images-contain">
                <div class="single-images-top" v-if="item.Pool?.length">
                    <div v-for="(value, index) in item.Pool">{{ value }}</div>
                </div>
                <!-- 中间中奖格子区 -->
                <div
        ...
        <template v-if="isFreeGame">
            <div class="single-images-event" v-if="item.isFrist && item.RoundWin">
        ...
        <div v-if="item.Plus" class="single-images-event">
            <span>{{ $t("Get") + item.Plus + " " + $t("BonusGame3") }}</span>
        </div>
    </div>
</template>
