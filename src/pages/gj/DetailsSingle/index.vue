        ...
        <div class="single-detail">
            <div class="single-detail-cell" v-for="(value, key) in detailFields" :key="key">
                ...
                <div :class="value.class ? value.class(formData[value.field]) : ''">
                    <span v-if="value.num">{{ formatNumber(formData[value.field]) }}</span>
                    <span v-else-if="value.html" v-html="value.html(formData[value.field])"></span>
                    <span v-else>{{
...
export default {
    ...
    computed: {
        detailFields() {
            return [
                ...
                { label: "i18_Win", field: "Win", num: true },
                {
                    label: "i18_Time",
                    field: "CreateTime",
    ...
    methods: {
        ...
        async initData() {
            ...
            console.log(!!this.decoded.hasOwnProperty("FreeTotalWin"), " ==========");
        ...
        formatTime,
        formatNumber(val) {
            console.log(val, Number(val).toFixed(2) * 1)
            // 统一数值格式化，去除多余小数点
