    ...
    <div class="single-images-list">
        ...
        <template v-if="item.PlateSymbolExtend">
            <div class="single-images-contain">
                <!-- 中间中奖格子区 -->
                <div class="single-images-mult" v-if="item.Wheel">
                    x&nbsp;&nbsp;{{ item.Wheel }}
                </div>
                <div
        ...
        <template v-else>
            <div class="single-images-event" v-if="item.Wheel">
                <span>{{ $t("All_Win_x") + item.Wheel }}</span>
            </div>
            <div v-if="item.RoundWin" class="single-images-event">
...
export default {
    ...
    computed: {
        ...
        gameTitle() {
            return this.keyValue === 0 ? this.$t("i18_BaseGame") : this.$t("Respin");
        },
    },
    mounted() {
