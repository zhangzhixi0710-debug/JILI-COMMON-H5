        ...
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <template v-if="item.PlateSymbolExtend">
                <div class="single-images-contain">
                    <!-- 中间中奖格子区 -->
                    <div class="single-images-mult" v-if="item.GetBonus && item.Mult">
                        x{{ item.Mult }}
                    </div>
            ...
            <div
                v-if="item.GetBonus && config.plates.length - 1 === index"
                class="single-images-event"
            >
                <span>{{ $t("ComboMultiple") }}</span>
                <span>{{ formatNumber(item.Mult) || 0 }}</span>
            </div>

            <div
                v-if="config.plates.length - 1 === index && item.RoundWin"
                class="single-images-event"
            >
