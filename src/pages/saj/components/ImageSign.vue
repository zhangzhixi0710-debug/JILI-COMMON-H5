<template>
    <div class="single-images-list">
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <template v-if="item.PlateSymbolExtend">
                <div class="single-images-contain">
                    ...
                    <div class="single-images-mult">
                        <div
                            v-for="(header, mIndex) in item.HeaderImage"
                            :key="mIndex"
                            :class="{ 'single-images-mult-active': item.Mult === header }"
                        >
                            <span>x</span>
                            <span>{{ header }}</span>
                        </div>
            ...
            <div
                v-if="config.isFrist && config.plates.length - 1 === index && config.AddRound"
                class="single-images-event"
            >
                <span>{{ $t("Get") + config.AddRound + $t("FreeGames") }}</span>
            </div>
        </div>
    </div>
</template>
