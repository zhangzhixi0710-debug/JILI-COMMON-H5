        ...
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <template v-if="item.PlateSymbolExtend">
                <div class="single-images-contain">
                    <!-- 中间中奖格子区 -->
                    <div class="single-images-mult" v-if="item.isLast">
                        x&nbsp;{{ config.Mult }}
                    </div>
                    <div class="single-images-middle" v-if="item.PlateSymbolExtend?.length">
                        <div
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    ...
                                    v-if="item.PlateSymbolNum?.[i]?.[imgIndex]"
                                    :class="{
                                    ...
                                    <img
                                        ...
                                        :style="{
                                            width:
                                                (
                                                    100 /
                                                    (item.PlateSymbolNum[i][imgIndex].length + 5)
                                                ).toFixed(0) + '%',
                                        }"
                                    />
                                </div>
            ...
            <div v-if="item.isLast && config.TotalWin" class="single-images-event">
                <span>{{ config.isBonus ? $t("BonusWin") : $t("MainGameWin") }}</span>
                <span>{{ formatNumber(config.TotalWin) }}</span>
            </div>
        </div>
    </div>
</template>
