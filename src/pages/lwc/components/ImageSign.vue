    ...
    <div class="single-images-list">
        ...
        <template v-if="item.PlateSymbolExtend">
            <div class="single-images-contain">
                <!-- 中间中奖格子区 -->
                <div class="single-images-mult" v-if="item.HeaderImage.length">
        ...
        <div v-if="item.GetFree" class="single-images-event">
            <span>{{ $t("Get") + 1 + $t("FreeGames2") }}</span>
        </div>
        <div v-if="isFreeGame && item.WheelMul" class="single-images-event">
            <span>{{ $t("Multiple") + item.WheelMul }}</span>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        gameTitle() {
            ...
            return this.keyValue === 0 ? this.$t("MainGame") : this.$t("FreeGame");
        },
    },
    mounted() {},
    methods: {
