<template>
    <div class="single-images-list">
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <div v-if="index === config.plates.length - 1" class="single-images-event">
                <span>{{ $t("PlateWin") }}</span>
                <span>{{ formatNumber(config.RoundWin) || 0 }}</span>
            </div>

            <div
                v-if="index === config.plates.length - 1 && config.FreeTotalRound"
                class="single-images-event"
            >
                <span>{{ $t("FreeGameMultiplier") + config.FreeTotalRound }}</span>
            </div>

            <div
                v-if="config.JPWinInfo && index === config.plates.length - 1"
                class="single-images-event"
            >
                <span v-if="config.JPWinInfo.JPType === 2">{{
                    $t("MinorScore") + config.JPWinInfo.JPTotalWin
                }}</span>
                <span v-if="config.JPWinInfo.JPType === 3">{{
                    $t("MajorScore") + config.JPWinInfo.JPTotalWin
                }}</span>
                <span v-if="config.JPWinInfo.JPType === 4">{{
                    $t("GrandScore") + config.JPWinInfo.JPTotalWin
                }}</span>
            </div>
        </div>
    </div>
...
export default {
    ...
    props: {
        ...
        config: {
            type: Object,
            required: false,
            default: () => ({ plates: [], RoundWin: 0, FreePlus: 0 }),
        },
