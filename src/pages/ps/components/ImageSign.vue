        ...
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <template v-if="item.PlateSymbolExtend">
                <div class="single-images-contain">
                    <!-- 中间中奖格子区 -->
                    <div class="single-images-mult" v-if="item.PartyResult.length">
                        <div v-for="(party, pIndex) in item.PartyResult" :key="pIndex">
                            <img v-if="party.img" :src="party.img" />
                        </div>
                    </div>
                    <div
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    class="single-images-slot-column"
                                    v-if="img.headerImg"
                                    :class="{
                                        'single-images-slot-column-active': isAwardActive(
                                            item.uuid,
                                            imagesIndex,
                                            imgIndex
                                        ),
                                ...
                                >
                                    <!-- 顶部数字图片（数组第一个元素） -->
                                    <div class="single-images-slot-counter">
                                        <img
                                            :src="img.headerImg"
                                            alt="counter"
                                            class="single-images-counter-img"
                                        />
                                    </div>
                                    <!-- 符号堆叠 -->
                                    <div
                                        class="single-images-slot-stack"
                                        :style="{ '--count': img.otherImg?.length || 0 }"
                                    >
                                        <img
                                            v-for="(im, imIndex) in img.otherImg"
                                            :key="imIndex"
                                            :src="im"
                                            class="single-images-symbol-img"
                                        />
                                    </div>
                                </div>
                                <div
            ...
            <div
                v-if="!config.isFrist && config.RoundWin && config.plates.length - 1 === index"
                class="single-images-event"
            >
