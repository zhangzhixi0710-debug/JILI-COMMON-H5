<template>
    <div class="single-container single">
        ...
        <div class="single-images">
            <SingleDetail  :config="RoundQueue.MainGame" :key="RoundQueue.uuid" />
            <MultipleDetail
                v-if="RoundQueue.rounds?.plates?.length"
                v-for="(item, key) in RoundQueue.rounds?.plates"
                :isFreeGame="isFreeGame"
                :key="item.uuid"
                :uuid="item.uuid"
                :config="item"
                :keyValue="key"
                :total="RoundQueue.rounds?.plates?.length"
            />
        </div>
...
import SingleDetail from "../components/single/index";
import MultipleDetail from "../components/multiple/index";

export default {
    name: "DetailsSingle",
    components: { SingleDetail, MultipleDetail },
    props: {
    ...
    methods: {
        ...
        async initData() {
            ...
            this.isFreeGame = !!this.decoded.hasOwnProperty("FreeTotalWin");
