    ...
    <div class="single-images-list">
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                ...
                                v-if="item.PlateSymbolNum[i]?.length"
                                :class="{
                                    'single-images-num-active': isAwardActive(i, imgIndex),
                                }"
                            >
                                <img
                                    v-for="(mul, mulIndex) in item.PlateSymbolNum[i][imgIndex]"
                                    :src="mul"
                                    :class="{
                                        'single-images-num-single':
                                            item.PlateSymbolNum[i][imgIndex][mulIndex],
                                        'single-images-num-single-dot': mul.includes('dot'),
                                    }"
                                    :style="{
                                        width:
                                            (100 / item.PlateSymbolNum[i][imgIndex].length).toFixed(
                                                0
                                            ) + '%',
                                    }"
                                />
        ...
        <div v-if="isFreeGame" class="single-images-event">
            <span v-if="item.RoundWin">{{
                $t(item.isFrist ? "MainGameWin" : "Respin_Win_min")
            }}</span>
        ...
        <div v-else-if="item.RoundWin" class="single-images-event">
            <span>{{ $t(item.isFrist ? "MainGameWin" : "Respin_Win_min") }}</span>
            <span>{{ formatNumber(item.RoundWin) }}</span>
        </div>

        <div v-if="item.DisplayID" class="single-images-event">
            <span>{{ $t("GetRespin") }}</span>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        gameTitle() {
            return this.item.isFrist ? this.$t("MainGame") : this.$t("Respin");
        },
    },
    mounted() {},
    methods: {
