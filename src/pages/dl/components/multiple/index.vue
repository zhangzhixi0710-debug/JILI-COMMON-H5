    ...
    <div class="single-images-list">
        ...
        <template v-if="config.PlateSymbolExtend">
            <div class="single-images-contain">
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                ...
                                v-if="isAwardBackActive(config.uuid, imagesIndex, imgIndex)"
                            ></div>
                        </div>
                    </div>

                    <div class="single-images-active-header" v-if="showHeader(config.uuid)">
                        <span v-if="changeList[config.uuid]?.Money">{{
                            formatNumber(changeList[config.uuid].Money)
                        }}</span>
                        <span>→</span>
                    </div>
                </div>
                ...
                <!-- 下方选项按钮 -->
                <div class="single-images-footer" v-if="config.AwardDataVec?.length">
                    <div
                        v-for="(opt, optI) in config.AwardDataVec"
                        :key="optI"
                        :class="{ 'single-footer-active': isFooterActive(config.uuid, opt.uuid) }"
                        @click.stop="handleClick(config.uuid, opt)"
                    >
                        <span>{{ opt.Index }}</span>
                    </div>
                </div>
        ...
        <div v-if="config.SingleWin" class="single-images-event">
            <span>{{ $t("single_details_freegamewin_mall") }}</span>
            <span>{{ formatNumber(config.SingleWin) || 0 }}</span>
        </div>

        <div v-if="config.GetFreeRound" class="single-images-event">
            <span>{{ $t("Get") + config.GetFreeRound + $t("FreeGames") }}</span>
        </div>
    </div>
</template>
