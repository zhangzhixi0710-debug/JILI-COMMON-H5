        ...
        <div v-for="(item, index) in config.plates" :key="index">
            ...
            <template v-if="item.PlateSymbolExtend">
                <div class="single-images-contain">
                    <!-- 中间中奖格子区 -->
                    <div class="single-images-mult">
                        <template v-if="item.RoundMultImages.length">
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    ...
                                    v-if="item.symbolNumArray[imagesIndex][imgIndex]"
                                    :class="{
                                ...
                                >
                                    <img
                                        v-for="(im, symbolNumIndex) in item.symbolNumArray[
                                            imagesIndex
                                        ][imgIndex].symbolNum"
                                        :key="symbolNumIndex"
                                        :class="{
                                        ...
                                        :style="{
                                            'max-width':
                                                (
                                                    100 /
                                                    (item.symbolNumArray[imagesIndex][imgIndex]
                                                        .symbolNum.length +
                                                        0.5)
                                                ).toFixed(0) + '%',
                                        }"
            ...
            <template v-if="config.isFrist">
                <div
                    v-if="config.plates.length - 1 === index && config.RoundWin"
            ...
            <template v-else>
                <template v-if="config.plates.length - 1 === index">
                    <div v-if="config.BonusWin" class="single-images-event">
                        <span>{{ $t("BonusWin") }}</span>
                        <span>{{ formatNumber(config.BonusWin) || 0 }}</span>
                    </div>

                    <div v-else-if="config.RoundWin" class="single-images-event">
            ...
            <div
                v-if="config.plates.length - 1 === index && config.IsGetFreeGame"
                class="single-images-event"
            >
...
export default {
    ...
    computed: {
        gameTitle() {
            ...
            if (this.config.symbolConversion) {
                return this.$t("symbol_conversion");
            }

            if (this.config.BoardReset) {
                return this.$t("BoardReset");
            }
