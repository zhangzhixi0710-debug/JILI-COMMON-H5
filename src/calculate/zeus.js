...
function processFreeQueue(freeInfo, gameId) {
    return freeInfo.map((roundData, roundIndex) => {
        ...
        const plates = ComboStageData.map(plate => {
            ...
            let result = ComboStageSymbol.map(row =>
                row.map(value => (SPECIAL === value ? plate.ChangeSpecial : value))
            );

            result = result.map(arr => {
                const filled = [...arr];
                while (filled.length < TARGET_LENGTH) {
                    filled.unshift(DEFAULT_IMG);
                }
                return filled;
            });
            ...
            const PlateSymbolExtend = generatePlateSymbolExtend(result, gameId);

            const AwardDataVec = (plate.AwardDataVec || []).map((award, idx) => {
                const { Symbol, Pos } = award;

                const { list, odds } = calcWinningColumnsRows(result, Symbol);
