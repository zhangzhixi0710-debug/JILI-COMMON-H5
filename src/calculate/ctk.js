const WILD_ID = [11];
const WILD_ID_2 = [11, 13, 14, 15, 16];
 ...
 */
function calcWinningColumnsRows(ComboStageSymbol, symbol, ComboStageMult) {
    var list = [];
    var continuous = true;
    const isMul = ComboStageMult && ComboStageMult > 1

    for (var colIndex = 0; colIndex < ComboStageSymbol.length; colIndex++) {
        ...
        for (var rowIndex = 0; rowIndex < column.length; rowIndex++) {
            ...
            if (continuous) {
                if (cell.Length && (sym === symbol || WILD_ID.includes(sym))) {
                    winningRows.push({ rowIndex: rowIndex, length: cell.Length });
                }
            }

            if (isMul && cell.Length && WILD_ID_2.includes(sym)) {
                winningRows.push({ rowIndex: rowIndex, length: cell.Length });
            }
        }
    ...
    // 首尾插入 0
    if (result.length) {
        const len = ComboStageSymbol.length
...
function processFreeQueue(freeInfo, gameId, isFree) {
    return freeInfo.map((roundData, roundIndex) => {
        ...
        const plates = ComboStageData.map((plate, plateIndex) => {
            ...
            const AwardDataVec = (plate.AwardDataVec || []).map(award => {
                ...
                // 计算中奖列索引（左到右连续）
                const { list, odds } = calcWinningColumnsRows(ComboStageSymbol, award.Symbol, plate.ComboStageMult);
...
export function processGameData(gameData, gameId) {
    ...
    let rounds = processFreeQueue(gameData.RoundQueue, gameId, gameData.isFree);
